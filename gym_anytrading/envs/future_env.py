...
class FuturesEnv(TradingEnv):
    ...
    def _calculate_reward(self, action):
        ...
        if self.check_if_close_trade(action):
            current_price = self.prices[self._current_tick]
            last_trade_price = self.prices[self._last_trade_tick]
            reward = 0

            if self._position == Positions.Long:
                reward = np.log(current_price / last_trade_price)
            elif self._position == Positions.Short:
                reward = np.log(last_trade_price / current_price)
    ...
    def _update_profit(self, action):
        ...
        if self.check_if_close_trade(action) or self._done:
            current_price = close_rev_scaling(self.prices[self._current_tick], self.sc, self.close_idx)
            last_trade_price = close_rev_scaling(self.prices[self._last_trade_tick], self.sc, self.close_idx)
            n_contracts = (self._total_profit * self.pos_size) / self.risk_per_contract

            if self._position == Positions.Long:
                pos_profit = ((current_price - last_trade_price) / last_trade_price) * self.point_value * n_contracts
                self._total_profit += pos_profit
            elif self._position == Positions.Short:
                pos_profit = ((last_trade_price - current_price) / current_price) * self.point_value * n_contracts
                self._total_profit += pos_profit
